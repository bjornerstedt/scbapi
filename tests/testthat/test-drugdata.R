...
test_that("GDP data", {
  gdp = get_scb("data/gdpreal", download_data = FALSE, returnList = TRUE)[["dataset"]]
...
test_that("Pop detail", {
  pop = get_scb("data/popdetail", returnList = TRUE)[["dataset"]]
  expect_equal(pop[["value"]][1], 3959115)
})
